"""Ollama proxy for load balancing across multiple Ollama instances."""
import asyncio
...
class OllamaProxy:
    """Load-balancing proxy for Ollama instances using round-robin."""
    ...
    async def call_ollama(self, model: str, prompt: str) -> Optional[str]:
        """Call Ollama with round-robin load balancing."""
    ...
    async def call_ollama_chat(self, model: str, messages: list[dict]) -> str:
        """Call Ollama chat endpoint with load balancing."""
        ...
        try:
            ...
            return "Sorry, error occurred while generating response."
