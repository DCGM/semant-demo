...
identify_language_answer_prompt_template = [
    ("system", 
    """
    You are a language detector. Your task is to identify the language of the text and respond with the corresponding ISO 639-2/T code  (e.g., 'ces', 'deu', 'eng', 'rus', 'slk')..
    Respond ONLY with a JSON object:
    {{"language": "ces"}} or {{"language": "eng"}} or {{"language": "deu"}} etc.
    """),
    ("user", "{text_string}")
...
eng_explain_selected_text_prompt_template = [
        ("system",
    """
...
cze_explain_selected_text_prompt_template = [
        ("system",
    """
    Jsi precizní a ochotný historický asistent. Uživatel ve tvé předchozí odpovědi označil konkrétní TVRZENÍ a chce vidět DŮKAZY z použitých pramenů.

    ZDROJE K PROHLEDÁNÍ:
    {context_string}

    CELÁ PŘEDCHOZÍ ODPOVĚĎ PRO KONTEXT:
    {full_answer}

    KONKRÉTNÍ TVRZENÍ K VYSVĚTLENÍ:
    "{selected_text}"

    INSTRUKCE:
    1) Hledej výhradně v POSKYTNUTÝCH ZDROJÍCH.
    2) Identifikuj konkrétní dokument [doc X] a přesnou větu, která dané TVRZENÍ přímo podporuje.
    3) Cituj relevantní větu z pramene v uvozovkách. Při vysvětlování odkazuj na zdroje důsledně pomocí formátu [doc X].
    4) Stručně vysvětli logickou souvislost mezi informací v prameni a tvrzením v odpovědi.
    5) Pokud tvrzení NENÍ v poskytnutých zdrojích podloženo, čestně to přiznej.
    6) Před napsáním věty se ujisti, že číslo [doc X] přesně odpovídá číslu v hlavičce zdroje. Nikdy nepoužívej čísla citací, která nejsou v seznamu zdrojů.

    """),
    MessagesPlaceholder(variable_name="prompt_history"),
    ("user", "Vysvětli, proč jsme uvedli: '{selected_text}'. Odpověz v jazyce dotazu a uveď CITACE.")
]