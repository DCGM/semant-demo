...
class TagRepository(WeaviateBaseRepository):
    ...
    async def create(self, collection_id: UUID, tag: PostTag) -> TagSchema:
        ...
        for obj in existing.objects:
            ...
            existing_examples = [str(item)
                                 for item in (props.get("tag_examples") or [])]
            if (
    ...
    async def update(self, tag_uuid: UUID, updated_tag: PatchTag) -> TagSchema:
        ...
        patch_data = updated_tag.model_dump(
        ...
        if "shorthand" in patch_data:
            properties_to_update["tag_shorthand"] = str(
                patch_data["shorthand"])
        if "color" in patch_data:
            properties_to_update["tag_color"] = str(patch_data["color"])
        if "pictogram" in patch_data:
            properties_to_update["tag_pictogram"] = str(
                patch_data["pictogram"])
        if "definition" in patch_data:
            properties_to_update["tag_definition"] = str(
                patch_data["definition"])
        ...
        if updated is None:
            raise NotFoundError(
                f"Tag with id {tag_uuid} not found after update")
        return updated
