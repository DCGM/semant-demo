        ...
        <q-card-section>
          <q-form @submit.prevent="onCreateTag">
            <div class="col q-col-gutter-md">
              ...
              <div class="row">
                <!-- current collection is selected by route / layout -->
                <q-input
    ...
    <q-form @submit.prevent="onTagManage">
      <div class="col q-col-gutter-md">
        ...
        <div class="row items-center q-mb-md">
          <div class="col">
            <div class="text-caption text-grey-6 q-mb-xs">Collection</div>
              ...
              <template v-slot:option="scope">
                <q-item v-bind="scope.itemProps">
                  <q-item-section avatar>
                    <AvatarItem
                      :annotation-class="{
                        short: scope.opt.shorthand || '?',
                        colorString: scope.opt.color || '#ccc',
                        textColor: 'black'
                      }"
                      size="sm"
                    />
...
const selectedCollection = computed(() => activeCollection.value)
...
async function onRunTask () {
  ...
  if (!selectedConfig.value) {
    Notify.create({ message: 'Please select a tagging configuration', color: 'negative' })
    loading.value = false
    return
  }
  for (const [index, entry] of tagFormManage.value.tag_uuids.entries()) {
    try {
      const tagValues = tags.value.find(t => t.id === entry.id)
      console.log('Tagging will start', tagValues)
      // const payload = { ...tagValues, tag_examples: tagValues?.examples.filter(example => example.trim() !== ''), task_config: selectedConfig.value }
      // const payload = { tag_name: tagValues?.name, tag_shorthand: tagValues?.shorthand, tag_color: tagValues?.color, tag_pictogram: tagValues?.pictogram, tag_definition: tagValues?.definition, tag_examples: tagValues?.examples.filter(example => example.trim() !== '') ?? [], task_config: selectedConfig.value }
      const payload = { tag_name: tagValues?.name, tag_shorthand: tagValues?.shorthand, tag_color: tagValues?.color, tag_pictogram: tagValues?.pictogram, tag_definition: tagValues?.definition, tag_examples: tagValues?.examples.filter(example => example.trim() !== '') ?? [], collection_name: selectedCollection.value?.name || tagForm.value.collection_name, task_config: selectedConfig.value }
      console.log('Payload: ', payload)
      console.log('Selected Config: ', selectedConfig.value)
      console.log('Tagging will start with payload:', payload)
      console.log('Tag name: ', tagValues?.name, 'Tag data: ', tagValues, 'Tag examples: ', tagValues?.examples.filter(example => example.trim() !== ''))
      const { data } = await api.post<TagStartResponse>('/tag/task', payload) // ('/tagging_task', payload)
      console.log('Tagging response received:', data)
...
async function fetchTags () {
  ...
  try {
    ...
    Notify.create({ message: 'Failed to fetch tags', color: 'negative' })
  } finally {
    loadingSpinnerTags.value = false
  }
}
...
watch(selectedCollection, async (newCollection) => {
  ...
  tagForm.value.collection_name = newCollection.name
  await fetchTags()
}, { immediate: true })
