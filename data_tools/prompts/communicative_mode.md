...
- If multiple classes apply, prefer the most specific class that describes what the passage is mainly doing.
...
# Possible classes
- `narration` — recounts events, actions, or happenings over time
- `description` — depicts persons, objects, places, situations, or qualities
- `exposition` — explains facts, concepts, causes, mechanisms, or background information
- `argumentation` — advances, supports, disputes, or interprets a claim or position
- `instruction` — tells how something should be done or what steps/rules should be followed
- `record` — documents facts, entries, transactions, proceedings, or observations in a documentary or evidentiary manner
- `interaction` — addresses another party in communicative exchange, such as requesting, replying, notifying, or directing
- `expression` — foregrounds feeling, reflection, devotion, praise, lament, or aesthetic verbal expression
...
- Prefer `interaction` over `instruction` when the passage is primarily addressed to a recipient as a request, reply, notice, or directive.
